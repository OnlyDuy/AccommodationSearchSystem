...
export class HomeViewPostsComponent extends AppComponentBase implements OnInit {
  ...
  ngOnInit() {
    this.updateTable();
    this.onPageChange({
      page: this.paginationParams.pageNum - 1,
      rows: this.paginationParams.pageSize,
    });
    this.onPageChangeVip({
      page: this.paginationParamsVip.pageNum - 1,
      rows: this.paginationParamsVip.pageSize,
    });
  }
  ...
  updateTable() {
    ...
    this.getAllVip(this.paginationParamsVip).subscribe((data) => {
      ...
      this.paginationParamsVip.totalPage = ceil(
        data.totalCount / this.maxResultCount
      );
  ...
  onPageChange(event: any) {
    ...
    this.getAll(this.paginationParams).subscribe((data) => {
      this.data = data.items;
      this.paginationParams.totalCount = data.totalCount;
      this.paginationParams.totalPage = Math.ceil(
        data.totalCount / this.maxResultCount
      );
    });
  }
  ...
  onPageChangeVip(event: any) {
    ...
    this.getAllVip(this.paginationParamsVip).subscribe((data) => {
      this.dataVip = data.items;
      this.paginationParamsVip.totalCount = data.totalCount;
      this.paginationParamsVip.totalPage = Math.ceil(
        data.totalCount / this.maxResultCount
      );
    });
  }
